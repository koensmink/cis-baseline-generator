...
The active boundary catalog contains host firewall, SMB session, LDAP channel, NTLM session, WinRM, RDP, endpoint malware protection, privileged credentials, application control, script control, generic transport cryptography, storage encryption, security audit, password-authentication strength, external identity authentication, weak/plaintext authentication, and privileged-role activation surfaces. Thirteen boundary sets define their minimum-effective combinations. Profile and deployment choices remain evaluation context, not generic catalog identity.
...
Twelve internal techniques represent authentication relay, adversary-in-the-middle behavior, remote-service abuse, network exposure, authentication downgrade, credential extraction, privilege elevation, malicious execution, defense impairment, audit suppression, plaintext interception, and access through missing encryption. Thirteen active attack paths include reviewed `AP-001` through `AP-010` plus paths for untrusted application execution, unencrypted transport, and unencrypted data at rest. `AP-010` has explicit threat and outcome references and is not an empty placeholder. Fourteen generic outcomes cover unauthorized access and authentication, credential theft, privilege escalation, lateral movement, code and malware execution, protection impairment, confidentiality, integrity, availability, broad administrative compromise, forensic evidence loss, and unauthorized data access.
