...
The active catalog contains 27 boundary definitions and 23 boundary-set
definitions. It covers the Windows reference surfaces, reusable identity and
authentication surfaces, and reusable application registration, consent,
service-principal authorization, and workload-identity trust surfaces. Profile
and deployment choices remain evaluation context, not generic catalog identity.

Catalog version 1.2.0 is the additive release-candidate catalog. It includes the
1.1 identity/authentication additions plus application registration, consent,
service-principal authorization, workload-identity trust, and canonical
security-effect normalization. These additions reuse existing capabilities and
do not place source-control identity in generic catalog objects.
...
The catalog contains 32 concrete threat scenarios. Every active scenario
contains actor position, preconditions, assets, weakness, action, objective,
immediate outcome, technical impact, boundary and technique references, High
confidence, evidence, and provenance.

The catalog contains 21 internal techniques, 26 active attack paths, and 14
generic security outcomes. `AP-001` through `AP-026` cover the validated Windows
and selected M365 reference slices; `AP-010` has explicit threat and outcome
references and is not an empty placeholder.
...
Current limitations are intentional: the catalog has no persistence layer, API,
UI, graph database, AI-generated authoritative mapping, CVE ingestion, full CWE
or ATT&CK ingestion, customer context, numeric risk score, or classifier
cutover. Atomic source mappings and benchmark-scoped shadow evaluations are
implemented, but remain advisory. Coverage reflects known mappings, not customer
residual risk.
